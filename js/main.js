...
async function analysis(menuId) {
    ...
    minerPopup.create();
    minerPopup.show();
    minerPopup.isCanBeSelect(content => content instanceof BaseFileInfo);
    minerPopup.isDefaultSelected(content => content instanceof FileInfo);
    minerPopup.download(async(lines) => await downloader.download(lines));

    await minerPopup.write(async() => await downloader.get());
}
...
function jav114Link(){
    ...
    if (url.includes('javdb.com/v/')){
        ...
        if (element){
            ...
            $(element).click(() => {
                let url = "https://www.141jav.com/search/" + id;
